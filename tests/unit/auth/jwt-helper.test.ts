...
describe('JWTHelper', () => {
  ...
  describe('decode', () => {
    ...
    it('should throw error for invalid token', () => {
      expect(() => jwtHelper.decode('invalid.token')).toThrow(/Failed to decode JWT/);
    });

    it('should decode token with custom claims', () => {
      const payload = { sub: '123', customClaim: 'value', nested: { data: 'test' } };
      const token = createToken(payload);
  ...
  describe('extractRoles', () => {
    ...
    it('should extract roles from custom namespace claim', () => {
      const payload = { sub: '123', 'https://lium.app/roles': ['admin', 'user'] };
      const token = createToken(payload);
  ...
  describe('extractPermissions', () => {
    it('should extract permissions from permissions claim', () => {
      const payload = { sub: '123', permissions: ['read:users', 'write:users'] };
      const token = createToken(payload);
