...
describe('ResultWriter', () => {
  ...
  describe('writeResult', () => {
    ...
    it('should write valid JSONL format', async () => {
      ...
      const files = await readdir(testResultsDir);
      const jsonlFiles = files.filter(f => f.endsWith('.jsonl'));
      const content = await readFile(join(testResultsDir, jsonlFiles[0]), 'utf-8');
  ...
  describe('writeResults', () => {
    ...
    it('should group results by pillar and environment', async () => {
      ...
      expect(jsonlFiles.some(f => f.includes('synthetic-staging'))).toBe(true);
    });
  ...
  describe('index file', () => {
    ...
    it('should update index with latest info', async () => {
      ...
      const indexContent = await readFile(join(testResultsDir, 'index.json'), 'utf-8');
