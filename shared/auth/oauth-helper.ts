...
export class OAuthHelper {
  ...
  async authenticateWithPassword(
    ...
    try {
      ...
      throw new Error(`OAuth password authentication failed: ${(error as Error).message}`);
    }
  }
  ...
  async authenticateWithClientCredentials(): Promise<TokenSet> {
    ...
    try {
      ...
      throw new Error(`OAuth client credentials authentication failed: ${(error as Error).message}`);
    }
  }
  ...
  async completeOAuthFlowInBrowser(
    ...
    try {
      ...
      // Fill in username
      const usernameField = await page.locator('input[type="email"], input[type="text"], input[name="username"]').first();
      ...
      await page.waitForURL(url => !url.toString().includes(this.config.issuer), {
        timeout: 10000,
      });
    } catch (error) {
  ...
  async exchangeCodeForTokens(code: string): Promise<TokenSet> {
    ...
    try {
      ...
      throw new Error(`OAuth code exchange failed: ${(error as Error).message}`);
    }
  }
