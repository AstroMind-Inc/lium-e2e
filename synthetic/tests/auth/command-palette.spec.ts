...
test.describe('Command Palette (CMD-K)', () => {
  test.describe('Admin User', () => {
    ...
    test('can search for "admin" and see results', async ({ page, envConfig }) => {
      // Navigate to app
      ...
      await page.screenshot({ path: 'reports/admin-cmd-k-search.png', fullPage: false });

      // Check for admin-related results
      // Adjust these selectors based on your app's structure
      const hasAdminResults = await page.locator('text=/admin/i').count() > 0 ||
  ...
  test.describe('Regular User', () => {
    ...
    test('cannot search for "admin" - no results shown', async ({ page, envConfig }) => {
      // Navigate to app
      await page.goto(envConfig.baseUrls.web);
      await page.waitForLoadState('networkidle');
      ...
      // Take screenshot for verification
      await page.screenshot({ path: 'reports/user-cmd-k-search.png', fullPage: false });
