...
test.describe('Multi-User Flow (Headless)', () => {
  test('Admin user can access admin features', async ({ browser, envConfig }) => {
    ...
    const hasAdminResults = await adminPage.locator('text=/admin/i').count() > 0;
  ...
  test('Regular user cannot access admin features', async ({ browser, envConfig }) => {
    ...
    const hasAdminResults = await userPage.locator('text=/admin/i').count() > 0;
  ...
  test('Both users can access common features', async ({ browser, envConfig }) => {
